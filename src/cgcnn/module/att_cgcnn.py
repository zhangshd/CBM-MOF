...
class CrystalGraphConvNet(nn.Module):
    ...
    def forward(self, batch):
        ...
        atom_fea = self.embedding_atom(batch['atom_fea'])
        atom_fea = self.embedding_atom_norm(atom_fea)

        if hasattr(self, 'embedding_extra'):
            extra_fea = self.embedding_extra(batch['extra_fea'])
            extra_fea = self.embedding_extra_norm(extra_fea)
            extra_fea = self.embedding_extra_softplus(extra_fea)
        
        for conv_func in self.convs:
            atom_fea = conv_func(atom_fea, batch['nbr_fea'], batch['nbr_fea_idx'])

        if self.att_pooling:
            # Pooling with attention mechanism
            crys_fea = self.atom_attention(atom_fea, batch['crystal_atom_idx'])
        else:
            # Pooling with mean pooling
            crys_fea = self.pooling(atom_fea, batch["crystal_atom_idx"])
        ...
        ret = {
            ...
            "cif_id": batch["cif_id"],
            "outs": outs,
            "last_layer_feas": last_layer_feas
        }
