"""Utilities for saving and rebuilding breakthrough-curve cache tables."""
...
def rebuild_curve_cache(bkt_dir: Path, output_csv: Path | None = None) -> pd.DataFrame:
    ...
    if not csv_paths:
        raise FileNotFoundError(f"No breakthrough_curve_data.csv found under {bkt_dir}")
    ...
    merged = merged.sort_values(["process", "mof", "time_min"], kind="stable").reset_index(drop=True)
