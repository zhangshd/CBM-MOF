...
# ---------------------------------------------------------------------------
# Constants from run_breakthrough.py
# ---------------------------------------------------------------------------
...
def load_bkt_summaries(bkt_dir: Path, eq_method: str = "competitive") -> pd.DataFrame:
    """Load and concatenate all bkt_summary_job*.csv files."""
    subdir = "summaries_iast" if eq_method == "IAST" else "summaries"
    files = sorted((bkt_dir / subdir / "jobs").glob("bkt_summary_job*.csv"))
    if not files:
        raise FileNotFoundError(
            f"No bkt_summary_job*.csv found in {bkt_dir / subdir / 'jobs'}"
        )
...
def load_breakthrough_curve_cache(bkt_dir: Path, eq_method: str = "competitive") -> pd.DataFrame:
    """Load cached breakthrough curves generated by run_breakthrough.py."""
    eq_tag = "iast_" if eq_method == "IAST" else ""
    cache_csv = bkt_dir / f"breakthrough_curves_data_{eq_tag.rstrip('_')}.csv" if eq_tag else bkt_dir / "breakthrough_curves_data.csv"

    # Auto-rebuild from per-run CSVs if cache doesn't exist
    if not cache_csv.exists():
        print(f"  Rebuilding curve cache from bkt_{eq_tag}* directories...")
        pattern = f"bkt_{eq_tag}*/*/breakthrough_curve_data.csv"
        csv_paths = sorted(bkt_dir.glob(pattern))
        if not csv_paths:
            raise FileNotFoundError(
                f"No breakthrough_curve_data.csv found matching {pattern} in {bkt_dir}"
            )
        dfs = []
        for p in csv_paths:
            df = pd.read_csv(p)
        ...
        merged = pd.concat(dfs, ignore_index=True).sort_values(
            ["process", "mof", "time_min"], kind="stable"
        ).reset_index(drop=True)
        merged.to_csv(cache_csv, index=False)
        print(f"  Saved: {cache_csv} ({len(merged)} rows, "
              f"{merged[['mof','process']].drop_duplicates().shape[0]} curves)")
        return merged
...
def step1_breakthrough_overlay(bkt_dir: Path, fig_dir: Path, eq_method: str = "competitive"):
    """Load cached C/C0 curves and generate the 2-panel breakthrough figure."""
    print("\n" + "=" * 70)
    print(f"STEP 1: Breakthrough Curve Overlay (eq_method={eq_method})")
    print("=" * 70)

    curve_df = load_breakthrough_curve_cache(bkt_dir, eq_method=eq_method)
...
def step2_performance_table(bkt_dir: Path, fig_dir: Path, eq_method: str = "competitive"):
    ...
    """
    print("\n" + "=" * 70)
    print(f"STEP 2: Performance Comparison Table (eq_method={eq_method})")
    print("=" * 70)

    bkt_df = load_bkt_summaries(bkt_dir, eq_method=eq_method)
    curve_df = load_breakthrough_curve_cache(bkt_dir, eq_method=eq_method)
...
def step3_selectivity_comparison(bkt_dir: Path, fig_dir: Path, eq_method: str = "competitive"):
    """Generate 2-panel dumbbell plot comparing GCMC, IAST, and BKT selectivities."""
    print("\n" + "=" * 70)
    print(f"STEP 3: Selectivity Comparison (eq_method={eq_method})")
    print("=" * 70)
...
def main():
    ...
    parser.add_argument(
        "--eq-method", type=str, default="competitive",
        choices=["competitive", "IAST"],
        help="Which BKT equilibrium results to use (default: competitive).",
    )
    parser.add_argument(
    ...
    if args.bkt_dir:
        ...
        md = bkt_dir.parent  # for fig_dir default
    else:
        if args.model_dir:
    ...
    print(f"Eq method: {eq_method}")

    if "1" in steps:
        step1_breakthrough_overlay(bkt_dir, fig_dir, eq_method=eq_method)
    if "2" in steps:
        step2_performance_table(bkt_dir, fig_dir, eq_method=eq_method)
    if "3" in steps:
        step3_selectivity_comparison(bkt_dir, fig_dir, eq_method=eq_method)
    if "4" in steps:
